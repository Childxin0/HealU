/**
 * 情绪分析 + 情境层（contextSignals）
 * 输出：primary/secondary、强度、置信度、contextType、keywordsMatched
 */

import { detectContext } from './contextSignals.js'
import { EMOTION_SEMANTIC_RULES, PATTERN_MULTIPLIERS } from './emotionSignals.js'
...
export function analyzeEmotion(text) {
  ...
  const { contextType, keywordsMatched } = detectContext(raw)
  ...
  for (const rule of EMOTION_SEMANTIC_RULES) {
    try {
      if (rule.test(raw)) {
  ...
  for (const pm of PATTERN_MULTIPLIERS) {
    try {
      if (pm.test(raw)) {
  ...
  if (raw.length > 140) scores.reflective += 8
  else if (raw.length > 60) scores.reflective += 4

  const intensWord = (raw.match(/真的|特别|非常|好|太|极其|快要/g) || []).length
  EMOTION_IDS.forEach((id) => {
    if (id !== 'reflective') scores[id] += intensWord * 3
  })
  ...
  if (primary === secondary) {
    secondary = sorted.find(([id]) => id !== primary)?.[0] || 'sad'
  }
  ...
  const intensity = Math.min(
    ...
    Math.round(
      22 +
        Math.min(boosted[primary], 48) * 1.35 +
        intensWord * 7 +
        raw.length * 0.06 +
        (contextType !== 'general' ? 6 : 0)
    )
  )
